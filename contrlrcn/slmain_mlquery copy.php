...
if ($_POST['doRegister'] and $_POST['fname'] and $_POST['username'] and $_POST['Institution'] and $_POST['phone'] and $_POST['email']) {
    ...
    if (empty($_SESSION['captcha_code']) || strcasecmp($_SESSION['captcha_code'], $_POST['captcha_code']) != 0) {
        ...
        if (!$totalUsers) {
            ...
            $smtpPortNo = "587"; // SMTP Port
            $smtpHost = "smtp.office365.com"; // SMTP Host
            $smtpUsername = "helpdesk@ncrst.na"; // SMTP username -- CHANGE --
            $smtpPassword = "Ta1ch@uk"; // SMTP password -- CHANGE --
            $emailTLS = "tls"; // Use TLS encryption
            $emailBcc = "gms@ncrst.na"; // BCC email address

            // Email content
            $fromEmail = "gms@ncrst.na";
            $fromName = "NCRST Grant Management";
            $subject = "NCRST Grant Management System - User Registration";
            $body = "<p>Dear $fname,<br>$lang_account_creation_message";
            ...
            //$mail->addBcc($emailBcc, $lang_grant_management_system);
            $mail->AddAddress($email, $sname); // To Address -- CHANGE --
            $mail->AddReplyTo($fromEmail, $lang_grants_management_system . "-" . $lang_UserRegistration); // Reply-To Address -- CHANGE --
...
<p>
<a href='https://$hostmain$path/confirm.php?uncrd=$master2&code=$refNo'>$lang_ClicktoActivateAccount</a>
</p>
...
";
            ...
            $message = '<p class="success" style="font-size:18px; padding:10px;">Dear ' . $fname . ' ' . $lang_accounthasbeensuccessfully . ' .</p>';
            $error = "success";
        } // end totatals
