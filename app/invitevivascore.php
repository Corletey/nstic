...
<body>
   ...
   <div class="success">Invite for VIVA</div>             
      <?php

$assignm_id=$_GET['assignm_id'];
$grantID=$_GET['grantID'];
$owner_id=$mysqli->real_escape_string($_GET['owner_id']);
$vivaToday=date("Y-m-d");
//doSaveFive
if($_POST['doSaveUndetake'] and $_POST['asrmApplctID'] and $id and $grantID and $_POST['vivadate'] and $_POST['vivadate']>=$vivaToday){

	$vivadate=$mysqli->real_escape_string($_POST['vivadate']);
...
$sqlSReviewer = "update ".$prefix."submissions_proposals set invitedforviva='$undertakereview' where owner_id='$owner_id' and projectID='$id' and grantcallID='$grantID'";
...
require_once("viewlrcn/mail_template_invite_viva.php");
...
/////////////////////////////Begin Mail Body
$mail->addCc("$emailBcc",$ReviewerName);//$emailBcc
//$mail->addBcc("",$ReviewerName);//$mail->addBcc($reviewerEmail,$ReviewerName);

$mail->FromName = "Invitation for VIVA"; //From Name -- CHANGE --
...
$mail->Subject = "Invitation for VIVA";
$body="$allSentMessage
";
$mail->MsgHTML($body);
...
if($_POST['doSaveUndetake'] and $_POST['asrmApplctID'] and $id and $grantID and $_POST['vivadate'] and $_POST['vivadate']<$vivaToday){
echo '<div class="btn-info-purple">VIVA Date should be in future, check date again and re-enter.</div>';	
...
<form action="" name="regForm" id="regForm" method="post" enctype="multipart/form-data" autocomplete="off">
						...
						<?php /*?>	<input name="undertakereview" type="radio" value="no" class="required" onChange="getUnderTakeReview(this.value)"> <?php echo $lang_No;?> <br><?php */?>
                            <input name="undertakereview" type="radio" value="yes" class="required" onChange="getUnderTakeReview(this.value)" checked> <?php echo $lang_Yes;?>
...
<div class="form-group row success">
<label>VIVA Date</label><br>
 <input type="date" name="vivadate" value="<?php echo $vivadate;?>" style=" width:80%; padding:5px;">
