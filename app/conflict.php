...
<body>
   ...
   <div class="success"><?php echo $lang_DeclareConflictofInterest;?></div>             
      <?php
$categorym=$_GET['categorym'];
if($categorym=='concepts'){															////////////////////subs///////////////
$sqlFLists1="SELECT *,DATE_FORMAT(`updatedon`,'%d/%m/%Y %H:%s:%i') AS updatedonm FROM ".$prefix."submissions_concepts where conceptID='$id' and category='concepts' order by conceptID desc";
...
$sqlFLists1="SELECT *,DATE_FORMAT(`updatedon`,'%d/%m/%Y %H:%s:%i') AS updatedonm FROM ".$prefix."submissions_proposals where projectID='$id'  and category='proposals' order by projectID desc";
$QueryFListsm1=$mysqli->query($sqlFLists1);
$rFLists2=$QueryFListsm1->fetch_array(); 
...
$sqlA2ProtocolaSS="update ".$prefix."conceptsasslogs_new set conflictofInterest='$conflict' where assignm_id='$assignm_id'";
...
/////////////////////////////Begin Mail Body
$mail->addCc("$emailBcc",$ReviewerName);
//$mail->addBcc("uncstuganda@gmail.com",$ReviewerName);//$mail->addBcc($reviewerEmail,$ReviewerName);
...
$mail->Subject = "Conflict of Interest - $public_title";
$body="$allSentMessage
";
$mail->MsgHTML($body);
...
<form action="" name="regForm" id="regForm" method="post" enctype="multipart/form-data" autocomplete="off">
                         ...
                         <div class="form-group row success">
							...
							<input name="conflict" type="radio" value="no" class="required"  onChange="getConflictofInterest(this.value)"> &nbsp;<?php echo $lang_no_conflict;?> &nbsp;&nbsp;<input name="conflict" type="radio" value="yes" class="required"  onChange="getConflictofInterest(this.value)"> <?php echo $lang_yes_conflict;?>
