...
$sqlUsers="SELECT * FROM ".$prefix."grantcall_categories where `categoryName`='$category' and `status`='new'  and categorym='concept' and catadmin_id='$sessionusrm_id' order by categoryID desc limit 0,1";
$QueryUsers = $mysqli->query($sqlUsers);
...
if($QueryUsers->num_rows and $category_number>=1){
echo $sqlA2="update ".$prefix."grantcall_categories set `category_number`='$category_number' where categoryID='$finalCategoryID' and status='new' and catadmin_id='$sessionusrm_id'";
$mysqli->query($sqlA2);		
...
$sqlCatNos="SELECT count(*) AS TotalCount FROM ".$prefix."grantcall_categories where `status`='new'  and categorym='concept' and catadmin_id='$sessionusrm_id' group by category_number order by TotalCount desc";
...
echo("<script>location.href = '$base_urlapp/data/DynamicCallConceptsQns/$id/';</script>");
}/////////////end
...
?><div class="tab">
    ...
    <button class="tablinks"  onclick="openCity(event, 'DynamicCallConcepts')" id="defaultOpen">Submit Concept Categories </button>

<button onClick="window.location.href='./data/DynamicCallConceptsQns/<?php echo $id;?>/'">Add Questions to Categories </button>
<button onClick="window.location.href='./data/SubmitCallforConceptNew/<?php echo $id;?>/'">Finish & Submit Call for Concept</button>
...
while($rUserProjectID2=$QueryProjectID2->fetch_array()){
	...
	/* category_name*/
$sqlProjectID3="SELECT * FROM ".$prefix."grantcall_categories where `status`='new'  and categorym='concept' and categoryName='$category_id' and catadmin_id='$sessionusrm_id' order by categoryID desc";
...
?>
  ...
  <?php if($category_id==18){?><input name="category_number[]" type="text" style="height:30px; width:100px;" value="<?php if(!$rUserProjectID3['category_number']){echo "1";}else{echo $rUserProjectID3['category_number'];}?>" readonly="readonly"/><?php }?>
  
       <?php if($category_id!=18){?><input name="category_number[]" type="text" style="height:30px; width:100px;" value="<?php echo $rUserProjectID3['category_number'];?>"/><?php }?>
	...
	<?php if($QueryProjectID3->num_rows and $rUserProjectID3['categoryName']!=18){?> <a href="./data/DynamicCallConcepts/<?php echo $rUserProjectID3['categoryID'];?>/"  style="color:#F00;padding:5px; font-size:12px;" onclick="return confirm('Are you sure you want to remove this Category?');">Remove Category</a><?php }?></td>
  </tr>
