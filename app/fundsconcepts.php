...
<body>
 ...
 $true=$mysqli->real_escape_string($_GET['true']);
...
//doSaveFive
if($_POST['doSaveAward'] and $id and $_POST['totalsubmitted'] and $grantID){
...
$sqlFLists1="SELECT *,DATE_FORMAT(`updatedon`,'%d/%m/%Y %H:%s:%i') AS updatedonm FROM ".$prefix."submissions_concepts where conceptID='$id' and grantcallID='$grantID' order by conceptID desc";
...
$sqlA2ProtocolaSS="update ".$prefix."request_for_funds_main set `actionOnRequest`='Approved',`BalanceonTotalBudget`='$BalanceonTotalBudget' ,`reason`='$reason' where conceptID='$pid' and id='$id' and grantID='$grantID'";
$mysqli->query($sqlA2ProtocolaSS);	

$sqlA2ProtocolaSS22="update ".$prefix."submissions_concepts set `GrantBalance`='$BalanceonTotalBudget' where conceptID='$pid' and grantcallID='$grantID'";
...
$sqlA2ProtocolaSS="update ".$prefix."request_for_funds_main set `actionOnRequest`='Rejected',`reason`='$reason' where conceptID='$pid' and id='$id' and grantID='$grantID'";
$mysqli->query($sqlA2ProtocolaSS);	

$sqlA2ProtocolaSS22="update ".$prefix."request_for_funds set `actionStatus`='Rejected' where conceptID='$pid' and mainFunds_id='$id' and grantID='$grantID'";
$mysqli->query($sqlA2ProtocolaSS22);	
...
$sqlUsers="SELECT * FROM ".$prefix."request_for_funds_main where `conceptID`='$pid' and id='$id' and grantID='$grantID' order by id desc limit 0,1";
$QueryUsers = $mysqli->query($sqlUsers);
$TotalFunds=$QueryUsers->num_rows;
$rpData=$QueryUsers->fetch_array();
$TotalBudget=$rpData['BudgetItem'];

$sqlUsers22="SELECT * FROM ".$prefix."request_for_funds where `conceptID`='$pid' and mainFunds_id='$id' and grantID='$grantID' order by id desc limit 0,1";
		$QueryUsers22 = $mysqli->query($sqlUsers22);
		$totalUsers22 = $QueryUsers22->num_rows;
		$rUserInv2=$QueryUsers22->fetch_array();
		
$wProposal="select * from ".$prefix."submissions_concepts where conceptID='$pid' and grantcallID='$grantID'";
...
?>
 ...
 <?php 
$sqlconceptID="SELECT * FROM ".$prefix."request_for_funds where `conceptID`='$pid'";
...
?>
        <tr>
            ...
            <td>
              <label><?php echo numberformat($rUserconceptID['EstimatedUnitCost']);?></label>
            </td>
            
            <td>
              <label><?php echo numberformat($rUserconceptID['TotalCOST']);?></label>
            </td>
