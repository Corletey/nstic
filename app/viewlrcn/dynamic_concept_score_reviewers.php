...
$usrrsLoggedId=$rsLogged['usrm_id'];
	...
	//////get all//////////////////////////
$queryContributionLogs="select * from ".$prefix."conceptsasslogs_new where conceptm_id='$conceptID' and conceptm_assignedto='$usrrsLoggedId'";
...
///////////////////////////////////////////////////////////////
$queryContribution="select * from ".$prefix."submissions_concepts where conceptID='$conceptID'";
$rs_Contribution=$mysqli->query($queryContribution);
$rsContribution=$rs_Contribution->fetch_array();
$conceptm_id=$rsContribution['conceptID'];
$conceptm_Times=$rsContribution['conceptm_Times'];
$conceptm_Times2=$rsContribution['conceptm_Times']+1;
//////////////////////////////////////////////////////////////////////////////////////
$queryContribution2="select * from ".$prefix."submissions_concepts where conceptID='$conceptID'";
...
//////////////////////////////////////////////////////////////////////////////
$sqlScoresMain = "SELECT * FROM ".$prefix."mscores_dynamic where conceptm_id='$conceptID' and EvaluatedBy='$conceptm_assignedto'";
...
$sqlScores = "SELECT * FROM ".$prefix."mscores_dynamic where conceptm_id='$conceptID' and EvaluatedBy='$conceptm_assignedto' and question_id='$finalCategoryID' and grantID='$grantcallID'";
...
values('$conceptID','$EvTotalScore','$sessionusrm_id',now(),'$ownermID','$scomment','$overallcomment','concepts','open','$finalCategoryID','$Verdict','$grantcallID')";
...
if($totalScores){
$queryScores="update ".$prefix."mscores_dynamic set `EvTotalScore`='$EvTotalScore',`scomment`='$scomment',`EvoverallComment`='$overallcomment',`Verdict`='$Verdict' where conceptm_id='$conceptID' and EvaluatedBy='$sessionusrm_id' and question_id='$finalCategoryID' and grantID='$grantcallID'";
$mysqli->query($queryScores);
...
$queryScores2="update ".$prefix."submissions_concepts  set `projectStatus`='Reviewed' where `conceptID`='$conceptID'";
$mysqli->query($queryScores2);
//}

if($conceptm_Times==0){
$queryScores2Times="update ".$prefix."submissions_concepts  set `conceptm_Times`='1' where `conceptID`='$conceptID'";
$mysqli->query($queryScores2Times);
////My Scores
$sqlScoresCons = "update ".$prefix."mscores_dynamic set `EvSame`='1' where conceptm_id='$conceptm_id' and EvaluatedBy='$conceptm_assignedto' and grantID='$grantcallID'";
$mysqli->query($sqlScoresCons);
}
if($conceptm_Times>=1){
$queryScores2Timesw="update ".$prefix."submissions_concepts  set `conceptm_Times`='$conceptm_Times2' where `conceptID`='$conceptID'";
$mysqli->query($queryScores2Timesw);
$sqlScoresConsd = "update ".$prefix."mscores_dynamic set `EvSame`='2' where conceptm_id='$conceptID' and EvaluatedBy='$conceptm_assignedto' and grantID='$grantcallID'";
$mysqli->query($sqlScoresConsd);

}

////////////////////update score logs///////////////////////////////////
$queryScores2d="update ".$prefix."conceptsasslogs_new  set `logm_status`='completed' where `conceptm_id`='$conceptID' and conceptm_assignedto='$conceptm_assignedto'";
$mysqli->query($queryScores2d);

$queryScores2dr="update ".$prefix."dynamic_concept_stages  set `status`='completed',`is_sent`='1' where `conceptID`='$conceptID' and reviewer_id='$conceptm_assignedto' and grantID='$grantcallID'";
...
$sqlUsers4="SELECT * FROM ".$prefix."submissions_concepts where conceptID='$conceptID' order by conceptID desc limit 0,1";
$QueryUsers4 = $mysqli->query($sqlUsers4);
$rUserInv4=$QueryUsers4->fetch_array();

//////////////////////////Score

$sqlUsersScore="SELECT * FROM ".$prefix."mscores_dynamic where conceptm_id='$conceptID' and EvaluatedBy='$sessionusrm_id'  and grantID='$grantcallID' order by scoredmID desc limit 0,1";
...
<div id="conceptScoreReviewerss" class="tabcontents">
 ...
 <input type="hidden" name="asrmApplctID" value="<?php echo $id;?>">
  ...
  <input type="hidden" name="conceptm_id" value="<?php echo $conceptID;?>">
...
<table width="100%" border="0" id="customers">
  ...
  <?php
  $qn_no=0;
  $sqlQuestions="SELECT * FROM ".$prefix."mscores_dynamic_qns where grantID='$grantcallID' order by qn_number asc";
$QueryQuestions = $mysqli->query($sqlQuestions);
while($rQuestions=$QueryQuestions->fetch_array()){
	
	$question_id=$rQuestions['id'];
	
	$sqlScores2 = "SELECT * FROM ".$prefix."mscores_dynamic where conceptm_id='$conceptID' and EvaluatedBy='$conceptm_assignedto' and question_id='$question_id'  and grantID='$grantcallID'";
