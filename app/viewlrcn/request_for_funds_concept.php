...
$sqlUsers="SELECT * FROM ".$prefix."request_for_funds where `owner_id`='$sessionusrm_id' and `conceptID`='$id' and mainFunds_id='$mainFunds_id' and actionStatus='Pending'  and BudgetItem='$BudgetItem' and grantID='$grantID' order by id desc limit 0,1";
...
values (NULL,'$id','$sessionusrm_id','$AmountofGrantawarded','$BudgetItem','$DescriptionofExpenditure','$TotalCost','$EstimatedUnitCost','$BalanceonTotalBudget',now(),'','Pending','$ProcurementPlanReference','$LocationforDelivery','$DateRequired','$mainFunds_id','Pending','0','$currency','$grantID')";
...
<?php
if($category=='RequestforFundsDelConcept' and $fid){
...
$sqlUsers="SELECT * FROM ".$prefix."request_for_funds_main where `conceptID`='$id' and grantID='$grantID' order by id desc limit 0,1";
$QueryUsers = $mysqli->query($sqlUsers);
$TotalFunds=$QueryUsers->num_rows;
$rpData=$QueryUsers->fetch_array();
$TotalBudget=$rpData['BudgetItem'];

$sqlUsers22="SELECT * FROM ".$prefix."request_for_funds where `owner_id`='$sessionusrm_id' and `conceptID`='$id' and grantID='$grantID' order by id desc limit 0,1";
...
<?php if($TotalFunds and $totalUsers22){?><div class="error2" style="text-transform:uppercase;">Your request of <b><?php echo numberformat($rpData['BudgetItem']);?> <?php echo $rUFunds['currency'];?></b> for <?php echo $rpData['requisitioning'];?> is pending completion. Click Submit Now to Finish.</div> <?php }?>
 ...
 <input type="hidden" name="conceptID" value="<?php echo $rUProposal['conceptID'];?>" >
  ...
  <input type="hidden" name="currency" value="<?php echo $rpData['currency'];?>" >
...
<option value="MatchingSupport">Matching Support if any</option>
    ...
    <?php 
$sqlProjectID="SELECT * FROM ".$prefix."request_for_funds where `owner_id`='$sessionusrm_id' and `conceptID`='$id'";
...
?>
        <tr>
            ...
            <td><a href="./main.php?option=RequestforFundsDelConcept&id=<?php echo $rUserProjectID['conceptID'];?>&fid=<?php echo $rUserProjectID['id'];?>&grantID=<?php echo $rUserProjectID['grantID'];?>"  style="background-color:#F00; color:#fff;padding:5px;" onclick="return confirm('Are you sure you want to delete this Budget Item?');">Remove </a></td>
            <td>&nbsp;</td>
        </tr>
        <?php }?>
    </table>
  ...
  <div class="row" style="padding-top:5px;">
    ...
    <?php if($totalUsers22){?><input type="button" name="doSubmit" value="<?php echo $lang_submit_now;?>"  onClick="window.location.href='./main.php?option=RequestforFundsFinalConcept&id=<?php echo $id;?>&grantID=<?php echo $grantID;?>'" style="margin-right:10px; float:right; padding:8px;" class="btn-info-purple"><?php }?>
