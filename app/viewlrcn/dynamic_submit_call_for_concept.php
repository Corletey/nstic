...
if($_POST['doSaveData'] and $_POST['title'] and $id){
...
//./files/  /app/files/
	    ...
	    $sqlUsers="SELECT * FROM ".$prefix."grantcalls where `grantID`='$id' order by grantID desc limit 0,1";
...
if($totalUsers){
	...
	logaction("$session_fullname $lang_Submitted_Call_for_Concepts");
...
require("mail_template_submit_call_concept.php");
...
$mail->AddReplyTo($adminusrm_email, $adminemail); //To Address -- CHANGE --
/////////////////////////////Begin Mail Body
$mail->AddAddress($adminusrm_email, $adminemail);
//$mail->addCc('$emailUsername','Activation Link from UNCST');//
//$mail->addCc("$emailBcc",'Call for Concept');//

$mail->WordWrap = 50; // set word wrap to 50 characters
$mail->IsHTML(false); // set email format to HTML
$mail->Subject = "$lang_grants_management_system - $lang_CallforConcepts";
$body="$allSentMessage
";
$mail->MsgHTML($body);
...
$mysqli->query($sqlA2);
echo "<meta http-equiv='REFRESH' content='0;url=$base_url/main.php?option=CallConcepts&id=$id'>";
}

		//////////////Update Now grantID
$sqlAUpdate="update ".$prefix."concept_dynamic_stages set `status`='old',`call_up`='1' where `grantcallID`='$id'";
$mysqli->query($sqlAUpdate);

$sqlAUpdate2="update ".$prefix."grantcall_categories set `status`='old' where `grantID`='$id'";
...
$sqlAUpdate23="update ".$prefix."concept_dynamic_questions_all_b set `is_sent`='1' where `grantID`='$id'";
$mysqli->query($sqlAUpdate23);

$sqlAUpdate24="update ".$prefix."concept_dynamic_questions_all_c set `is_sent`='1' where `grantID`='$id'";
$mysqli->query($sqlAUpdate24);

$sqlAUpdate25="update ".$prefix."concept_dynamic_questions_all_d set `is_sent`='1' where `grantID`='$id'";
$mysqli->query($sqlAUpdate25);	

$sqlAUpdate26="update ".$prefix."concept_dynamic_questions_all_e set `is_sent`='1' where `grantID`='$id'";
$mysqli->query($sqlAUpdate26);

$sqlAUpdate2y="update ".$prefix."concept_dynamic_questions_all_f set `is_sent`='1' where `grantID`='$id'";
$mysqli->query($sqlAUpdate2y);

$sqlAupdate_call="update ".$prefix."grantcall_questions set `status`='old' where `grantID`='$id'";
$mysqli->query($sqlAupdate_call);


$sqlA2="update ".$prefix."grantcalls set  `title`='$title',`summary`='$summary',`shortacronym`='$shortacronym',`details`='$details',`requirefullproposal`='$requirefullproposal',`startDate`='$startDate',`EndDate`='$EndDate',`category`='concepts',gnt_hour='$gnt_hour',gnt_hour_end='$gnt_hour_end',`publish`='$publish',`dynamic`='Yes' where `grantID`='$id'";
$mysqli->query($sqlA2);
echo "<meta http-equiv='REFRESH' content='0;url=$base_url/main.php?option=CallConcepts&id=$id'>";

$message='<p class="success">'.$lang_dear.' '.$session_fullname.'"'.$lang_new_details_saved.'"</p>';
...
//check any category
$sqlCatGrantCategoryUp="SELECT * FROM ".$prefix."concept_dynamic_stages where `grantcallID`='$id'  and categorym='concept' and catadmin_id='$sessionusrm_id' and questions_up=1 order by id desc";
...
<button <?php if($AnyCategoryRows['questions_up']=='1'){?>class="tablinks"<?php }?> class="tablinks" onClick="window.location.href='./main.php?option=DynamicCallConceptsQns&id=<?php echo $id;?>&action=update'"><?php echo $lang_new_AddQuestionsCategories;?> </button>
<button <?php if($AnyCategoryRows['call_up']=='1'){?>class="tablinks"<?php }?> onclick="openCity(event, 'SubmitCallforConceptNew')" id="defaultOpen"><?php echo $lang_new_FInishSubmitCOncept;?></button>
<?php }?>

</div>

<div id="SubmitCallforConceptNew" class="tabcontent">
  <span onclick="this.parentElement.style.display='none'" class="topright">&times</span>

   
  <h3><?php echo $lang_CallforConcepts;?></h3>
...
/*$sqlProjectID2r_title="SELECT * FROM ".$prefix."grantcall_questions where `categoryID`='1' and `status`='new' and `project_title`='Yes' order by questionID desc limit 0,1";
$QueryProjectID2r_title = $mysqli->query($sqlProjectID2r_title);
$QueryProjectID2r_title->num_rows;
if(!$QueryProjectID2r_title->num_rows){
	echo $messagefff='<p class="error">Dear '.$session_fullname.', you cant submit a call because you have not added Project Question.</p>';
	
	echo '<meta http-equiv="refresh" content="5; URL='.$base_url.'main.php?option=DynamicCallConceptsQns/$id/" />';
  ...
  ?>
    ...
    <div class="col-100">
    <label for="fname"><?php echo $lang_new_CallTitle;?> <span class="error">*</span></label>
  ...
  <div class="row success">
    ...
    <div class="col-100">
    <label for="lname">3. <?php echo $lang_new_ProvideDetails;?> <span class="error">*</span></label>
 ...
 <option value="01" <?php if($chops13=='01' || $chops13=='01' || $chops13=='01'){?>selected="selected"<?php }?>>&nbsp;01</option>
   ...
   <div class="row success">
    ...
    <div class="col-100" style="font-size:18px; font-weight:bold;">
    <label for="fname"><?php echo $lang_new_DoesthisCallrequireProposal;?>  <span class="error">*</span></label>
     <input name="requirefullproposal" type="radio" value="Yes" class="" required <?php if($rUserInv2['requirefullproposal']=='Yes'){?>checked="checked"<?php }?>/> <?php echo $lang_Yes;?>
     <input name="requirefullproposal" type="radio" value="No" class="" required <?php if($rUserInv2['requirefullproposal']=='No'){?>checked="checked"<?php }?>/> <?php echo $lang_No;?>
    </div>
  </div>

  <div class="row success">
    <input type="submit" name="doSaveData" value="<?php echo $lang_new_FinishSubmit;?>">
  </div>
