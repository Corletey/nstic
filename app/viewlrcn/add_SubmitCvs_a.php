...
?><!-- Content Header (Page header) -->
                <section class="content-header">
                    <h1>
                           <small><?php echo $sitename;?> - Please attach cvs</small>
                    </h1>
                    <ol class="breadcrumb">
...
$attachment_d = $usrm_id."d_".$mysqli->real_escape_string(preg_replace('/\s+/', '_', $_FILES['attachment_d']['name']));
$targetw_d = "files/". basename($usrm_id."d_".preg_replace('/\s+/', '_', $_FILES['attachment_d']['name']));
$studytoolsext_maind = move_uploaded_file(preg_replace('/\s+/', '_', $_FILES['attachment_d']['tmp_name']), $targetw_d);
$extension_txtm_d = getExtension($filed_main_d);
$extension_txtm_d = strtolower($filed_main_d);
...
if($_FILES['attachment_d']['name']){
$queryd="insert into ".$prefix."concepts_cvs (`usrm_id`,`cvname`,`cvdate`) 
values('$usrm_id','$attachment_d',now())";
$mysqli->query($queryd);
}
...
echo("<script>location.href = './main.php?option=SubmitCvb';</script>");
...
<table width="100%" border="0">
    ...
    <td width="26%">Upload File (CV)</td>
    <td width="74%">
    <input type="file" name="attachment_d" id="attachment_d"/>  <br />  
...
<div class="progress-bar progress-bar-info progress-bar-striped" role="progressbar" aria-valuenow="40" aria-valuemin="0" aria-valuemax="100" style="width:40%">30% Complete</div>
</div>
<p style="font-weight:bold;"><span style="color:#F00; font-size:16px;">Important:</span> You will be required to submit a minimum of four (4) CVs and maximum of ten (10) CVs. These can be uploaded later</p>
