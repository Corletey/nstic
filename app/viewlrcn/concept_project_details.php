<?php
 if($_POST['doSaveData']=='Save and Next' and $_POST['Methodology'] and $_POST['asrmApplctID'] and $id){
...
$ExpectedIntellectualProperty.=$_POST['ExpectedIntellectualProperty'][$i].'-';
	...
	$sqlUsers="SELECT * FROM ".$prefix."project_details_concept where `owner_id`='$asrmApplctID' and `is_sent`='0' and grantcallID='$id' order by id desc limit 0,1";
...
$sqlUsers2="SELECT * FROM ".$prefix."project_primary_beneficiaries where `Categoryofbeneficiary`='$Categoryofbeneficiary' and `Gender`='$Gender' and`owner_id`='$asrmApplctID' and is_sent='0' and grantcallID='$id' order by id desc";
...
echo '<img src="img/loading_wait.gif">';
echo '<div class="spacer"></div>';
echo "<meta http-equiv='REFRESH' content='0;url=$base_url/main.php?option=conceptBudget&id=$id'>";
...
$r= $cmdwb->fetch_array();
if($totalStages){
$sqlASubmissionStages="update ".$prefix."concept_stages  set `ProjectDetails`='1' where `owner_id`='$asrmApplctID' and `conceptID`='$conceptm_id' and status='new' and grantcallID='$id'";
...
$sqlUsers2="SELECT * FROM ".$prefix."project_details_concept where `owner_id`='$usrm_id' and `is_sent`='0' and grantcallID='$id' order by id desc limit 0,1";
$QueryUsers2 = $mysqli->query($sqlUsers2);
...
<?php
$sqlUsers3="SELECT * FROM ".$prefix."project_primary_beneficiaries where `owner_id`='$usrm_id' and `is_sent`='0' and grantcallID='$id' order by id desc limit 0,40";
...
<td><a href="./main.php?option=conceptProjectDetailsDel&id=<?php echo $rUserInv3['id'];?>" style="color:#F00; font-weight:bold;" onclick="return confirm('Are you sure you want to delete this item?');">Delete</a></td>
</tr>
...
<input name="ExpectedIntellectualProperty[]" type="checkbox" value="None" <?php if($chop1=='None' || $chop2=='None' || $chop3=='None' || $chop4=='None' || $chop5=='None' || $chop6=='None'){?>checked="checked"<?php }?>/> <?php echo $lang_new_None;?>
  ...
  <div class="row success">
    ...
    <div class="col-100">
    <label for="lname"><strong>Total Budget Cost</strong> </label><br />
    ...
    <select name="currencyPrimaryFunder" id="currencyPrimaryFunder" style="float:right; width:30%;">
...
<table id="customers2" border="0">
   ...
   <option value="">&nbsp;Please Select</option>
    <option value="Months" <?php if($rUserInv2['periodPrimaryFunder']=='Months'){?>selected="selected"<?php }?>>&nbsp;Months</option>
    
    <option value="Years" <?php if($rUserInv2['periodPrimaryFunder']=='Years'){?>selected="selected"<?php }?>>&nbsp;Years</option>
...
<table id="customers2" border="0">
   ...
   <option value="">&nbsp;Please Select</option>
    <option value="Months" <?php if($rUserInv2['periodSecondaryFunder']=='Months'){?>selected="selected"<?php }?>>&nbsp;Months</option>
    
    <option value="Years" <?php if($rUserInv2['periodSecondaryFunder']=='Years'){?>selected="selected"<?php }?>>&nbsp;Years</option>
...
<table id="customers2" border="0">
   ...
   <option value="">&nbsp;Please Select</option>
    <option value="Months" <?php if($rUserInv2['periodCounterpart']=='Months'){?>selected="selected"<?php }?>>&nbsp;Months</option>
    
    <option value="Years" <?php if($rUserInv2['periodCounterpart']=='Years'){?>selected="selected"<?php }?>>&nbsp;Years</option>
