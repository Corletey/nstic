<h4 class="niceheaders">Concepts Scheduled for Review</h4>
<?php
$page='main.php?';
$url='category=';
$value='AdminApprovedReview&id='.$id;
...
*/
if($_POST['doSearch']){
$query = $mysqli->query("select COUNT(*) as num from ".$prefix."submissions_concepts where projectStatus='Scheduled for Review' order by conceptID desc");//and conceptm_status='new' 
}
...
/* Get data. */
if($_POST['doSearch']){
$sql = "select *,DATE_FORMAT(`updatedon`,'%d/%m/%Y') AS updatedonm FROM ".$prefix."submissions_concepts  where projectStatus='Scheduled for Review' order by conceptID desc LIMIT $start, $limitm";//and conceptm_status='new' 
}
...
<a href="./main.php?option=reviewProjectInformation&id=<?php echo $rFLists2['conceptID'];?>" class='button3'><?php echo $lang_ViewSubmission;?></a>
