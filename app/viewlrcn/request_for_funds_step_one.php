...
$sqlUsers="SELECT * FROM ".$prefix."request_for_funds_main where `projectID`='$projectID' and `conceptID`='$conceptID' and `owner_id`='$owner_id'  and grantID='$grantID' order by id desc limit 0,1";
$QueryUsers = $mysqli->query($sqlUsers);

if(!$totalUsers = $QueryUsers->num_rows  and $_POST['AmountofGrantRequesting']<=$_POST['TotalCost']){
$Insert_QR2="insert into ".$prefix."request_for_funds_main (`projectID`,`conceptID`,`owner_id`,`ApprovedGrantTotal`,`BudgetItem`,`DescriptionofExpenditure`,`TotalCOST`,`BalanceonTotalBudget`,`dateRequested`,`receivedBy`,`actionOnRequest`,`requisitioning`,`is_sent`,`currency`,`reason`,`grantID`) values ('$projectID','$conceptID','$owner_id','$AmountofGrantawarded','$BudgetItem','$DescriptionofExpenditure','$TotalCost','$BalanceonTotalBudget',now(),'0','Pending','$amountvalue','0','$currency',NULL,'$grantID')";
...
echo "<meta http-equiv='REFRESH' content='0;url=$base_url/main.php?option=RequestforFunds&id=$id&grantID=$grantID'>";
}
...
if($totalUsers = $QueryUsers->num_rows  and $_POST['AmountofGrantRequesting']<=$_POST['TotalCost']){
$Insert_QR2="update ".$prefix."request_for_funds_main  set `requisitioning`='$amountvalue',`BudgetItem`='$BudgetItem',`TotalCOST`='$TotalCost' where conceptID='$conceptID' and grantID='$grantID'";
$mysqli->query($Insert_QR2);
...
$wProposal="select * from ".$prefix."submissions_proposals where projectID='$id' and grantcallID='$grantID'";
...
$sqlFunds="SELECT * FROM ".$prefix."request_for_funds_main where projectID='$id' and `owner_id`='$rowner_id'  and grantID='$grantID' order by id desc limit 0,1";
...
$sqlFundsPendingApproval="SELECT * FROM ".$prefix."request_for_funds_main where projectID='$id' and `owner_id`='$rowner_id'  and grantID='$grantID' and actionOnRequest='Submitted' order by id desc limit 0,1";
