<?php
if($_POST['doSaveData']=='Save and Next' and $_POST['Personnel'] and $_POST['PersonnelTotal'] and $_POST['asrmApplctID'] and $id){
...
$Personnel=$mysqli->real_escape_string($_POST['Personnel']);
	...
	$sqlUsers="SELECT * FROM ".$prefix."concept_budget where `owner_id`='$asrmApplctID' and `is_sent`='0' and grantcallID='$id' order by id desc limit 0,1";
...
$r= $cmdwb->fetch_array();
if($totalStages){
$sqlASubmissionStages="update ".$prefix."concept_stages  set `Budget`='1' where `owner_id`='$asrmApplctID' and `conceptID`='$conceptm_id' and status='new' and grantcallID='$id'";
...
echo "<meta http-equiv='REFRESH' content='0;url=$base_url/main.php?option=newconceptReferences&id=$id'>";
...
$sqlUsers2="SELECT * FROM ".$prefix."concept_budget where `owner_id`='$usrm_id' and `is_sent`='0' and grantcallID='$id' order by id desc limit 0,1";
...
<div class="tab">
 ...
 <button <?php if($rUConceptStages['ProjectInformation']==1){?>class="tablinks"<?php }?> onClick="window.location.href='./main.php?option=newSubmitConcept/<?php echo $id;?>/'"><?php echo $lang_new_ProjectInformation;?></button>
 
  <button <?php if($rUConceptStages['PrincipalInvestigator']==1){?>class="tablinks"<?php }?> onClick="window.location.href='./main.php?option=newconceptPrincipalInvestigator/<?php echo $id;?>/'"><?php echo $lang_new_ProjectTeam;?></button>
  
    <button <?php if($rUConceptStages['Introduction']==1){?>class="tablinks"<?php }?>  onClick="window.location.href='./main.php?option=newconceptIntroduction/<?php echo $id;?>/'"><?php echo $lang_new_Introduction;?></button>
    
   <button <?php if($rUConceptStages['ProjectDetails']==1){?>class="tablinks"<?php }?> onClick="window.location.href='./main.php?option=newconceptProjectDetails/<?php echo $id;?>/'"><?php echo $lang_new_ProjectDetails;?></button>
   
  <button <?php if($rUConceptStages['Budget']==1){?>class="tablinks"<?php }?>  onclick="openCity(event, 'newconceptBudget')" id="defaultOpen"><?php echo $lang_new_Budget;?></button>
  
  <button <?php if($rUConceptStages['cReferences']==1){?>class="tablinks"<?php }?> onClick="window.location.href='./main.php?option=newconceptReferences/<?php echo $id;?>/'"><?php echo $lang_new_Citations;?></button>
  
   <button <?php if($rUConceptStages['conceptAttachments']==1){?>class="tablinks"<?php }?> onClick="window.location.href='./main.php?option=newconceptAttachments/<?php echo $id;?>/'">Attachments </button>
</div>
 <?php
$sqlUsersBudget="SELECT * FROM ".$prefix."project_details_concept where `owner_id`='$usrm_id' and `is_sent`='0' and grantcallID='$id' order by id desc limit 0,1";
...
?>
<?php 
  $sqlDynamic="SELECT * FROM ".$prefix."concept_dynamic_questions_all_e where grantID='$id' order by id asc";
...
function fnc(value, min, max) 
{
    ...
    <input type="text" id="Personnel" name="Personnel" placeholder="The personnel costs (salary top ups) should not be detailed by person but by position" class="requiredm number" value="<?php if($rUserInv2['Personnel']){echo $rUserInv2['Personnel'];}else{ echo $_POST['Personnel'];}?>" maxlength="<?php echo $mmPersonnelTotal;?>" required min="1" max="<?php echo $mmPersonnelTotal;?>" onkeyup="imposeMinMax(this)">Kyengo
    
    
  onkeyup="this.value = fnc(this.value, 0, <?php echo $mmPersonnelTotal;?>);"
