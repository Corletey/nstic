...
$usrrsLoggedId=$rsLogged['usrm_id'];
	...
	//////get all//////////////////////////
$queryContributionLogs="select * from ".$prefix."conceptsasslogs_new where conceptm_id='$conceptID' and conceptm_assignedto='$usrrsLoggedId' and categorym='concepts'";
...
//////////////////////////////////////////////////////////////////////////////
$sqlScoresMain = "SELECT * FROM ".$prefix."mscores_dynamic where conceptm_id='$conceptID' and EvaluatedBy='$conceptm_assignedto' and categorym='concepts'";
...
$sqlScores = "SELECT * FROM ".$prefix."mscores_dynamic where conceptm_id='$conceptID' and EvaluatedBy='$conceptm_assignedto' and grantID='$grantcallID' and question_id='$finalCategoryID' and categorym='concepts'";
...
if($totalScores){
$queryScores="update ".$prefix."mscores_dynamic set `EvTotalScore`='$EvTotalScore',`scomment`='$scomment',`EvoverallComment`='$overallcomment',`Verdict`='$Verdict' where conceptm_id='$conceptID' and EvaluatedBy='$sessionusrm_id' and question_id='$finalCategoryID' and grantID='$grantcallID' and categorym='concepts'";
$mysqli->query($queryScores);
...
$sqlUsers4="SELECT * FROM ".$prefix."submissions_concepts where conceptID='$conceptID' and grantcallID='$grantcallID' order by conceptID desc limit 0,1";
$QueryUsers4 = $mysqli->query($sqlUsers4);
$rUserInv4=$QueryUsers4->fetch_array();

//////////////////////////Score

$sqlUsersScore="SELECT * FROM ".$prefix."mscores_dynamic where conceptm_id='$conceptID' and EvaluatedBy='$sessionusrm_id'  and grantID='$grantcallID'   and categorym='concepts' order by scoredmID desc limit 0,1";
...
<table width="100%" border="0" id="customers">
  ...
  <?php
  $qn_no=0;
$sqlQuestions="SELECT * FROM ".$prefix."mscores_dynamic_qns where grantID='$grantcallID'  and categorym='concepts' order by qn_number asc";
$QueryQuestions = $mysqli->query($sqlQuestions);
while($rQuestions=$QueryQuestions->fetch_array()){
	
	$question_id=$rQuestions['id'];
	
	$sqlScores2 = "SELECT * FROM ".$prefix."mscores_dynamic where conceptm_id='$conceptID' and EvaluatedBy='$conceptm_assignedto' and question_id='$question_id'  and grantID='$grantcallID'  and categorym='concepts'";
...
function imposeMinMax(el){
  if(el.value != ""){
    ...
    <input name="EvTotalScore[]" type="number" class="requiredm number" id="QnewMethods" autocomplete="off" value="<?php echo $rScores2['EvTotalScore'];?>" min="1" max="<?php echo $rQuestions['percentScore'];?>" onkeyup="imposeMinMax(this)"/> </td>
