...
$url='proposalNewSubmissionsMain&id='.$id;
$value='';
...
*/
if($_POST['doSearch']){
$query = $mysqli->query("select COUNT(*) as num from ".$prefix."submissions_proposals where (finalSubmission='Made Final Submission' || finalSubmission='Pending Final Submission')  order by conceptID desc");//and conceptm_status='new' 
}
...
/* Get data. */
if($_POST['doSearch']){
$sql = "select *,DATE_FORMAT(`updatedon`,'%d/%m/%Y') AS updatedonm FROM ".$prefix."submissions_proposals  where (finalSubmission='Made Final Submission' || finalSubmission='Pending Final Submission') order by conceptID desc LIMIT $start, $limitm";//and conceptm_status='new' 
}
...
?><strong style="font-size:24px; font-weight:bold; color:#F00;"><?php echo ($TotalScore);?>%</strong><br />
<?php 
...
<?php if($rowsGrants['end_review']=='Yes'){?>
