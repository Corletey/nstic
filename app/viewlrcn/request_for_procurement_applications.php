...
<h2 class="success" style="text-align:center;"><b><?php echo $lang_RequestforProcurement;?></b></h2>
 
 <?php 
$category=$_POST['category'];
$page='main.php?';
$url='category=';
$value='ProcurementApplications&id='.$id;
...
$query = $mysqli->query("select COUNT(*) as num from ".$prefix."request_for_procurement where owner_id='$usrm_id' order by id desc");//and conceptm_status='new' 
}

if($session_usertype!='user'){
$query = $mysqli->query("select COUNT(*) as num from ".$prefix."request_for_procurement order by id desc");//and conceptm_status='new' 
}
...
/* Get data. */
if($session_usertype=='user'){
$sql = "select *,DATE_FORMAT(`dateRequested`,'%d/%m/%Y') AS dateRequestedm FROM ".$prefix."request_for_procurement where owner_id='$usrm_id' order by id desc LIMIT $start, $limitm";//and conceptm_status='new' 
}

if($session_usertype!='user'){
$sql = "select *,DATE_FORMAT(`dateRequested`,'%d/%m/%Y') AS dateRequestedm FROM ".$prefix."request_for_procurement order by id desc LIMIT $start, $limitm";//and conceptm_status='new' 
}
...
if($projectID>=1){
$sqlwwScores2 = "select * FROM ".$prefix."submissions_proposals where projectID='$projectID' order by projectID desc limit 0,10";
$resultwwScores2 = $mysqli->query($sqlwwScores2);
$rFListsScores2=$resultwwScores2->fetch_array(); 
}else{

$sqlwwScores2 = "select * FROM ".$prefix."submissions_concepts where conceptID='$conceptID' order by conceptID desc limit 0,10";
$resultwwScores2 = $mysqli->query($sqlwwScores2);
$rFListsScores2=$resultwwScores2->fetch_array(); 
}
															  ...
															  ?>
                                                    <tr>
                                                      <td class="small-col"><?php echo $rFListsScores2['projectTitle'];?><br />
...
<?php if($rFLists2['actionOnRequest']=='Pending'){?><div  class='button3'><?php echo $lang_RequestPending;?></div> <?php }?>
 ...
 <?php if($rFLists2['actionOnRequest']=='Rejected'){?><div  class='btn-info-purple'><?php echo $lang_RequestPending;?></div> <?php }?>
 <?php if($rFLists2['actionOnRequest']=='Rejected with Comments'){?><div  class='btn-info-purple'><?php echo $lang_RequestPending;?></div> <?php }?>                                            
                                                        
                                                        
                                                      </td>
                                                      <td class="time">
                                                      
  <?php if($rFLists2['actionOnRequest']=='Pending'){?><div  class='button3'><?php echo $lang_PendingFinalSubmission;?></div> <?php }?>
  <?php if($rFLists2['actionOnRequest']=='Submitted'){?><div  class='button3'><?php echo $lang_RequestSubmitted;?></div> <?php }?>
                                                        
  <?php if($rFLists2['actionOnRequest']=='Submitted' and $session_usertype=='admin'){?>
    <input id="go" type="button" value="<?php echo $lang_approve_rejectProcurement;?>" onclick="window.open('<?php echo $base_url;?>funds.php?pid=<?php echo $rFLists2['projectID'];?>&id=<?php echo $rFLists2['id'];?>&categorym=proposals&owner_id=<?php echo $owner_id;;?>','popUpWindow','height=500, width=1200, left=100, top=100, resizable=yes, scrollbars=yes, toolbar=yes, menubar=no, location=no, directories=no, status=yes');"  class="button2" >
