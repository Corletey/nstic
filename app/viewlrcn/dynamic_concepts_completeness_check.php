 ...
 <h4 class="niceheaders"><?php echo $lang_CompletenessCheck;?></h4><hr />
 <?php 
 $sessionusrm_id=$_SESSION['usrm_id'];
$category=$_POST['category'];
$page='main.php?';
$url='category=';
$value='CompletenessCheck&id='.$id;
...
$query = $mysqli->query("select COUNT(*) as num from ".$prefix."submissions_concepts where (projectStatus='Completeness Check-Approved' || projectStatus='Completeness Check-Rejected' || projectStatus='Completeness Check-Resubmission') and owner_id='$sessionusrm_id' order by conceptID desc");//and conceptm_status='new' 
}

if($session_usertype!='user'){
$query = $mysqli->query("select COUNT(*) as num from ".$prefix."submissions_concepts where (projectStatus='Completeness Check-Approved' || projectStatus='Completeness Check-Rejected' || projectStatus='Completeness Check-Resubmission') order by conceptID desc");//and conceptm_status='new' 
}
...
/* Get data. */
if($session_usertype=='user'){
$sql = "select *,DATE_FORMAT(`updatedon`,'%d/%m/%Y') AS updatedonm FROM ".$prefix."submissions_concepts  where (projectStatus='Completeness Check-Approved' || projectStatus='Completeness Check-Rejected' || projectStatus='Completeness Check-Resubmission')  and owner_id='$sessionusrm_id' order by conceptID desc LIMIT $start, $limitm";//and conceptm_status='new' 
}

if($session_usertype!='user'){
$sql = "select *,DATE_FORMAT(`updatedon`,'%d/%m/%Y') AS updatedonm FROM ".$prefix."submissions_concepts  where (projectStatus='Completeness Check-Approved' || projectStatus='Completeness Check-Rejected' || projectStatus='Completeness Check-Resubmission') order by conceptID desc LIMIT $start, $limitm";//and conceptm_status='new' 
}
...
$sqlwwScores = "select conceptm_id,usrm_id,count(EvaluatedBy) AS TotalReviewers,sum(EvTotalScore) AS Total FROM ".$prefix."mscores_new where usrm_id='$owner_id' and conceptm_id='$conceptID'  and categorym='concepts' group by conceptm_id order by conceptm_id desc limit 0,10";
...
$color="#C0E4C2";
                                                    ...
                                                    <tr>
                                                      ...
                                                      <td class="name"><?php if($base_lang=='en'){echo $rCategory['rstug_categoryName'];}
if($base_lang=='fr'){echo $rCategory['rstug_categoryName_fr'];}
if($base_lang=='pt'){echo $rCategory['rstug_categoryName_pt'];}?></td>
                                                        <td class="subject"  style="background:<?php echo $color;?>!important;">
                                                        <?php echo $lang_phone_Name;?>: <?php echo $rFListsOnwner['usrm_fname'];?> <?php echo $rFListsOnwner['usrm_sname'];?><br />
                                                        <?php echo $lang_email;?>: <?php echo $rFListsOnwner['usrm_email'];?><br />
                                                        <?php echo $lang_phone_number;?>: <?php echo $rFListsOnwner['usrm_phone'];?>                                                        </td>
                                                        <td class="time">
  ...
  <?php if($rFLists2['projectStatus']=='Scheduled for Review'){?><div class="btn-info-blue"><?php echo $lang_ScheduledforReview;?></div><?php }?>
