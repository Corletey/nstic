...
<span  class="button2" style="float:left; padding:10px;"><a href="exportconfproposals.php"> <?php echo $lang_ExportResults;?></a></span>
 
 <?php 
$category=$_POST['category'];
$page='main.php?';
$url='category=';
$value='ConflictofInterestProposals&id='.$id;
...
*/
if($_POST['doSearch']){
$query = $mysqli->query("select COUNT(*) as num from ".$prefix."conceptsasslogs_new where conflictofInterest='Yes' and categorym='proposals' order by assignm_id desc");//and conceptm_status='new' 
}
...
/* Get data. */
if($_POST['doSearch']){
$sql = "select *,DATE_FORMAT(`assignm_date`,'%d/%m/%Y') AS assignm_datem FROM ".$prefix."conceptsasslogs_new where conflictofInterest='Yes' and categorym='proposals' order by assignm_id desc LIMIT $start, $limitm";//and conceptm_status='new' 
}
...
$queryproposal="select * from ".$prefix."submissions_proposals where projectID='$conceptm_id'";
...
$sqlwwScores = "select *,count(EvaluatedBy) AS TotalReviewers,sum(EvTotalScore) AS Total FROM ".$prefix."mscores_new where usrm_id='$owner_id' and conceptm_id='$conceptID'  and categorym='proposals' group by conceptm_id order by scoredmID desc limit 0,10";
...
$color="#C0E4C2";
                                                 ...
                                                 <?php echo $rFLists2['assignm_datem'];?>                                                        </td>
                                                      <td class="time">
