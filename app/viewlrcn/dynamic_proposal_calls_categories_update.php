...
if ($_POST['doSaveUpdate'] == 'Save Ordering' and $_SESSION['usrm_id'] and $id) {
  ...
  for ($i = 0; $i < count($_POST['finalCategoryID']); $i++) {
    ...
    $sqlA2 = "update " . $prefix . "grantcall_categories set `category_number`='$category_number' where `categoryID`='$finalCategoryID' and grantID='$id' and catadmin_id='$sessionusrm_id'";
    $mysqli->query($sqlA2);

    /////Update Stages
    $sqlAStatus = "update " . $prefix . "concept_dynamic_stages set `catordering_up`='1' where `grantcallID`='$id' and catadmin_id='$sessionusrm_id'";
  ...
  $sqlCatNos = "SELECT count(*) AS TotalCount FROM " . $prefix . "grantcall_categories where `grantID`='$id'  and categorym='proposal' and catadmin_id='$sessionusrm_id' group by category_number order by TotalCount desc";
...
echo("<script>location.href = '".$base_url."'main.php?option=DynamicCallProposalQns/$id/';</script>");
}/////////////end*/
} //end Post	\
...
if ($_POST['doNextPage'] == 'Proceed to Next Page') {
  echo '<img src="./img/ajax-loader1.gif">';
  echo '<div class="spacer"></div>';
  echo "<meta http-equiv='REFRESH' content='0;url=$base_url/main.php?option=DynamicCallProposalQns&id=$id&action=update'>";
}
...
?><div class="tab">
  ...
  if ($AnyCategorySavedG) { ?>
    ...
    <button <?php if ($AnyCategoryRows['catordering_up'] == '1') { ?>class="tablinks" <?php } ?> onclick="openCity(event, 'DynamicCallProposalsUpdate')" id="defaultOpen"><?php echo $lang_new_UpdateOrdering; ?> </button>
...
<div id="DynamicCallProposalsUpdate" class="tabcontent ">
  ...
  <form action="" method="post" name="regForm" id="regForm" autocomplete="off" enctype="multipart/form-data">
    ...
    <div class="container success"><!--begin-->
      ...
      <div class="row success">
        ...
        $sqlProjectID2 = "SELECT * FROM " . $prefix . "grantcall_categories where `grantID`='$id'  and categorym='proposal' and catadmin_id='$sessionusrm_id' order by categoryID desc";
        ...
        if ($QueryProjectID2->num_rows) { ?>
            ...
            ?>
              <tr>
                ...
                <td width="28%" align="center" style="border-bottom:2px solid #4CAF50; border-right:2px solid #4CAF50;">
                  ...
                  <?php if ($categoryName == 1) { ?><input name="category_number[]" type="text" style="height:30px; width:100px;" value="<?php if (!$rUserProjectID3['category_number']) {
                                                                                                                                      echo "1";
                                                                                                                                    } else {
                                                                                                                                      echo $rUserProjectID3['category_number'];
          ...
          <input type="submit" name="doSaveUpdate" value="Save Ordering">
