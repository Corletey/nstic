...
$sqlASubmissionStages="insert into ".$prefix."progress_report_review (`projectID`,`progressID`,`owner_id`,`SignaturePage`,`Abstract`,`SummaryofScientificProgress`,`KeyPersonnelEffort`,`Publications`,`PatentsandLicenses`,`status`,`reviewer_id`)  values('$id','$progressID','$owner_id','0','0','0','0','1','0','new','$sessionusrm_id')";
...
$sqlASubmissionStages="update ".$prefix."progress_report_review  set `Publications`='1' where `owner_id`='$owner_id' and status='new' and reviewer_id='$sessionusrm_id'";
$mysqli->query($sqlASubmissionStages);
}
}


$sqlUsers2="SELECT * FROM ".$prefix."progress_meeting_abstracts where `owner_id`='$owner_id'  and  `progressID`='$progressID' order by meetingAbstractID desc limit 0,1";//conceptID='$conceptID'
$QueryUsers2 = $mysqli->query($sqlUsers2);
$totalUsers = $QueryUsers2->num_rows;
$rUserInv2=$QueryUsers2->fetch_array();
?>
<div class="tab">
  ...
  <button <?php if($rUConceptStages['SummaryofScientificProgress']==1){?>class="tablinks"<?php }?> onClick="window.location.href='./main.php?option=reviewSummaryofScientificProgress&id=<?php echo $id;?>'"><?php echo $lang_SummaryofScientificProgress;?> </button>
    ...
    <button <?php if($rUConceptStages['Publications']==1){?>class="tablinks"<?php }?>  onclick="openCity(event, 'reviewPublications')" id="defaultOpen"><?php echo $lang_Publications;?></button>
...
<div id="reviewPublications" class="tabcontent">
...
<label for="fname"><?php echo $lang_new_RequiredFieldsMarked;?> (<span class="error">*</span>)</label>
    ...
    <div class="col-100">
    <label for="lname">
<?php echo $lang_Publications;?>
 
<span class="error">*</span></label><br />
<strong><?php echo $rUserInv2['chapters'];?></strong>
    </div>
  </div>
 ...
 <div class="row success">
    ...
    <div class="col-100">
    <label for="lname">
MEETING ABSTRACTS (name of meeting, date, location, title, author, abstract or page #)
...
<table width="100%" border="0" id="POITable">
      ...
      <?php
$sqlUsers2="SELECT * FROM ".$prefix."progress_meeting_abstracts where `owner_id`='$owner_id'  and  `progressID`='$progressID' order by meetingAbstractID desc limit 0,40";//conceptID='$conceptID'
$QueryUsers2 = $mysqli->query($sqlUsers2);
...
<td><?php echo $rUserInv2['MeetingDate'];?></td>
  ...
  <div class="row success">
    ...
    <div class="col-100">
    <label for="lname">
OTHER PRESENTATIONS (Organization, location, date, title of presentations)
...
<table width="100%" border="0" id="POITable2">
  ...
  <?php
$sqlUsers23="SELECT * FROM ".$prefix."progress_report_otherpresentations where `owner_id`='$owner_id'  and  `progressID`='$progressID' order by id desc limit 0,40";//conceptID='$conceptID'
