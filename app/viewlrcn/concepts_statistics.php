<?php
$sqlPending="SELECT *  FROM ".$prefix."submissions_concepts where projectStatus='Pending Final Submission' order by conceptID desc";
$QueryPending = $mysqli->query($sqlPending);
$Pendings=$QueryPending->num_rows;


$sqlScheduled="SELECT *  FROM ".$prefix."submissions_concepts where projectStatus='Scheduled for Review' order by conceptID desc";
$QueryScheduled = $mysqli->query($sqlScheduled);
$Scheduled=$QueryScheduled->num_rows;

$sqlApproved="SELECT *  FROM ".$prefix."submissions_concepts where projectStatus='Approved' order by conceptID desc";
$QueryApproved = $mysqli->query($sqlApproved);
$Approved=$QueryApproved->num_rows;

$sqlReviewed="SELECT *  FROM ".$prefix."submissions_concepts where projectStatus='Reviewed' order by conceptID desc";
$QueryReviewed = $mysqli->query($sqlReviewed);
$Reviewed=$QueryReviewed->num_rows;

$sqlRejected="SELECT *  FROM ".$prefix."submissions_concepts where projectStatus='Rejected' order by conceptID desc";
