...
$wmOwner="select * from ".$prefix."dynamic_concept_titles where  dconceptID='$dconceptID'";
...
//////////////////////////////////////////////////////////////////////////////
$sqlScoresMain = "SELECT * FROM ".$prefix."mscores_dynamic where conceptm_id='$dconceptID' and EvaluatedBy='$conceptm_assignedto' and openstatus='open'";
...
$sqlScores = "SELECT * FROM ".$prefix."mscores_dynamic where conceptm_id='$dconceptID' and EvaluatedBy='$conceptm_assignedto' and openstatus='open' and question_id='$finalCategoryID' and grantID='$id'";
...
$queryScores2="update ".$prefix."dynamic_concept_titles  set `projectStatus`='Reviewed',`conceptm_Reviewers`='$noReviewed' where `dconceptID`='$dconceptID'";
$mysqli->query($queryScores2);
//}
if($conceptm_Times==0){
$queryScores2Times="update ".$prefix."dynamic_concept_titles  set `conceptm_Times`='1' where `dconceptID`='$dconceptID'";
$mysqli->query($queryScores2Times);
////My Scores
$sqlScoresCons = "update ".$prefix."mscores_dynamic set `EvSame`='1' where conceptm_id='$conceptm_id' and EvaluatedBy='$conceptm_assignedto' and grantID='$id'";
$mysqli->query($sqlScoresCons);
}
if($conceptm_Times>=1){
$queryScores2Timesw="update ".$prefix."dynamic_concept_titles  set `conceptm_Times`='$conceptm_Times2' where `dconceptID`='$dconceptID'";
$mysqli->query($queryScores2Timesw);
$sqlScoresConsd = "update ".$prefix."mscores_dynamic set `EvSame`='2' where conceptm_id='$dconceptID' and EvaluatedBy='$conceptm_assignedto' and grantID='$id'";
$mysqli->query($sqlScoresConsd);

}
////////////////////update score logs///////////////////////////////////
$queryScores2d="update ".$prefix."conceptsasslogs_new  set `logm_status`='completed' where `conceptm_id`='$dconceptID' and conceptm_assignedto='$conceptm_assignedto'";
$mysqli->query($queryScores2d);

$queryScores2dr="update ".$prefix."dynamic_concept_stages  set `status`='completed',`is_sent`='1' where `dconceptID`='$dconceptID' and reviewer_id='$conceptm_assignedto' and grantID='$id'";
...
<p>
This is to inform you that $ms_NameOfPI's Concept was Evaluated by $usrm_fname, find below the full details:</p>
<p>
...
$sqlUsersScore="SELECT * FROM ".$prefix."mscores_dynamic where conceptm_id='$dconceptID' and EvaluatedBy='$sessionusrm_id'  and openstatus='open'  and grantID='$id' order by scoredmID desc limit 0,1";
...
while($rQuestions=$QueryQuestions->fetch_array()){
	...
	$sqlScores2 = "SELECT * FROM ".$prefix."mscores_dynamic where conceptm_id='$dconceptID' and EvaluatedBy='$conceptm_assignedto' and question_id='$question_id'  and grantID='$id'";
