...
values('$category','$category_number','Yes','$sessionusrm_id','dynamic')";
$mysqli->query($sqlA2);	
...
values('$title','$summary','$details','$attachmentFile2','$startDate','$EndDate','concepts',NULL,'$shortacronym','Yes','$sessionusrm_id','No')";
$mysqli->query($sqlA2);
...
$sqlSaved="SELECT * FROM ".$prefix."concept_dynamic_stages where categorym='concept' and catadmin_id='$sessionusrm_id' and grantcallID='$idgrantID'  and grantcallID>=1  order by id desc limit 0,1";
...
values('concept','1','0','0','0','0','0','0','0','0',now(),'new','$newGrantID','$sessionusrm_id')";
...
$sqlUsers="SELECT * FROM ".$prefix."grantcall_categories where `categoryName`='$category' and categorym='concept' and catadmin_id='$sessionusrm_id' and grantID='$idgrantID' order by categoryID desc limit 0,1";
$QueryUsers = $mysqli->query($sqlUsers);
...
values('$category','concept',now(),'new','$newGrantID','1','$sessionusrm_id','dynamic')";//$category_number
$mysqli->query($sqlA2);	
$message='<p class="success">Dear '.$session_fullname.', details have been submitted, please add another category.</p>';
//$newGrantID=$mysqli->insert_id;
...
/*echo("<script>location.href = '".$base_url."'main.php?option=DynamicCallConcepts&id=".$newGrantID."&action=update';</script>");*/
echo "<meta http-equiv='REFRESH' content='0;url=$base_url/main.php?option=DynamicCallConcepts&id=$newGrantID&action=update'>";
}
...
$sqlUsers="SELECT * FROM ".$prefix."grantcall_categories where `categoryName`='$category' and categorym='concept' and catadmin_id='$sessionusrm_id' and grantID='$id' order by categoryID desc limit 0,1";
$QueryUsers = $mysqli->query($sqlUsers);
if(!$QueryUsers->num_rows and $category){
echo $sqlA2="insert into ".$prefix."grantcall_categories (`categoryName`,`categorym`,`date_added`,`status`,`grantID`,`category_number`,`catadmin_id`,`catadminstatus`) 

values('$category','concept',now(),'new','$id','$category_number','$sessionusrm_id','dynamic')";
$mysqli->query($sqlA2);	
$message='<p class="success">Dear '.$session_fullname.', details have been submitted, please add another category.</p>';
...
/*echo("<script>location.href = '".$base_url."'main.php?option=DynamicCallConcepts&id=".$id."&action=update';</script>");*/
echo "<meta http-equiv='REFRESH' content='0;url=$base_url/main.php?option=DynamicCallConcepts&id=$id&action=update'>";
...
$sqlCatNos="SELECT * FROM ".$prefix."grantcall_categories where categorym='concept' and catadmin_id='$sessionusrm_id' and grantID='$id' order by categoryID desc";
...
/*echo("<script>location.href = '".$base_url."'main.php?option=DynamicCallConceptsUpdate&id=".$id."&action=update';</script>");*/
echo "<meta http-equiv='REFRESH' content='0;url=$base_url/main.php?option=DynamicCallConceptsUpdate&id=$id&action=update'>";
	}else{
...
//check any category
$sqlCatGrantCategoryUp="SELECT * FROM ".$prefix."concept_dynamic_stages where categorym='concept' and catadmin_id='$sessionusrm_id' and grantcallID='$id' order by id desc";
...
?><div class="tab">
    ...
    <button <?php if($AnyCategoryRows['categories_up']=='1'){?>class="tablinks"<?php }?>  onclick="openCity(event, 'DynamicCallConcepts')" id="defaultOpen"><?php echo $lang_new_SubmitConceptCategories;?> </button>
...
if($QueryProjectID2->num_rows){?>
 ...
 <tr>
   <th><input type="checkbox" onclick="toggle(this);" /><?php echo $lang_new_Enable;?></th>
...
while($rUserProjectID2=$QueryProjectID2->fetch_array()){
	...
	/* category_name*/
$sqlProjectID3="SELECT * FROM ".$prefix."grantcall_categories where categorym='concept' and categoryName='$category_id' and catadmin_id='$sessionusrm_id' and grantID='$id' and grantID>=1 order by categoryID desc";
...
<?php /*?><input name="finalCategoryID[]" type="hidden" value="<?php echo $rUserProjectID3['categoryID'];?>" /><?php */?>
    ...
    <td width="47%" style="border-bottom:2px solid #4CAF50; border-right:2px solid #4CAF50;"><?php if($base_lang=='en'){echo $rUserProjectID2['category_name'];} if($base_lang=='fr'){echo $rUserProjectID2['category_name_fr'];} if($base_lang=='pt'){echo $rUserProjectID2['category_name_pt'];} ?></td>
    
    
    <td width="18%" align="center" style="border-bottom:2px solid #4CAF50;"> 
      
      
      <?php if($QueryProjectID3->num_rows and $rUserProjectID3['categoryName']!=1){?> <a href="./main.php?option=DynamicCallConcepts&id=<?php echo $id;?>&catID=<?php echo $rUserProjectID3['categoryID'];?>&action=update"  style="color:#F00;padding:5px; font-size:12px;" onclick="return confirm('Are you sure you want to remove this Category?');"><?php echo $lang_new_RemoveCategory;?></a><?php }?></td>
  </tr>
