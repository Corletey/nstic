...
<h4 class="niceheaders">Reviewed Proposals </h4><hr />
 <?php 
$category=$_POST['category'];
$page='main.php?';
$url='category=';
$value='ApprovedProposalsAdmin&id='.$id;
...
*/
if($_POST['doSearch']){
$query = $mysqli->query("select COUNT(*) as num from ".$prefix."submissions_concepts where projectStatus='Reviewed' order by conceptID desc");//and conceptm_status='new' 
}
...
/* Get data. */
if($_POST['doSearch']){
$sql = "select *,DATE_FORMAT(`updatedon`,'%d/%m/%Y') AS updatedonm FROM ".$prefix."submissions_concepts  where projectStatus='Reviewed' order by conceptID desc LIMIT $start, $limitm";//and conceptm_status='new' 
}
...
if($dynamic=='Yes'){
$sqlwwScores = "select *,sum(EvTotalScore) AS Total FROM ".$prefix."mscores_dynamic where usrm_id='$owner_id' and conceptm_id='$conceptID'  and categorym='concepts' group by conceptm_id order by scoredmID desc";
$resultwwScores = $mysqli->query($sqlwwScores);
$rFListsScores=$resultwwScores->fetch_array();
...
$color="#C0E4C2";
  ...
  <?php
if($dynamic=='No'){
$queryCategoryReview="select * from ".$prefix."mscores_new where conceptm_id='$conceptID' and usrm_id='$owner_id'  and categorym='concepts' group by EvaluatedBy order by scoredmID";
...
if($dynamic=='Yes'){
$queryCategoryReview="select * from ".$prefix."mscores_dynamic where conceptm_id='$conceptID' and usrm_id='$owner_id'  and categorym='concepts' group by EvaluatedBy order by scoredmID";
$R_CategoryReview=$mysqli->query($queryCategoryReview);	
while($rCReview=$R_CategoryReview->fetch_array()){
	 $EvaluatedBy=$rCReview['EvaluatedBy']; 
...
$rFListsReviewer=$R_DReviewer->fetch_array();
	...
	?>
                                                        <?php if($TotalScore and $rFListsReviewer['usrm_sname']){?>
  <input id="go" type="button" value="<?php echo $rFListsReviewer['usrm_sname'];?> <?php echo $rFListsReviewer['usrm_fname'];?>, View Score" onclick="window.open('scoresheetdynamic.php?id=<?php echo $EvaluatedBy;?>&ds=<?php echo $rCReview['scoredmID'];?>&grantID=<?php echo $rCReview['grantID'];?>&dconceptID=<?php echo $conceptID;?>','popUpWindow','height=700, width=1000, left=100, top=100, resizable=yes, scrollbars=yes, toolbar=yes, menubar=no, location=no, directories=no, status=yes');"  class="btn-info-blue" ><br /><?php }?>                                                
