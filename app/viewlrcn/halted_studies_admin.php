...
$page='main.php?';
$url='category=';
$value='HaltedStudiesAdmin&id='.$id;
...
*/
if($_POST['doSearch']){
$query = $mysqli->query("select COUNT(*) as num from ".$prefix."submissions_proposals where haltstudy='Yes' order by projectID desc");//and conceptm_status='new' 
}
...
/* Get data. */
if($_POST['doSearch']){
$sql = "select *,DATE_FORMAT(`updatedon`,'%d/%m/%Y') AS updatedonm FROM ".$prefix."submissions_proposals  where haltstudy='Yes' order by projectID desc LIMIT $start, $limitm";//and conceptm_status='new' 
}
...
$rFListsStudy=$RStudy->fetch_array();
   ...
   <?php if($rFLists2['projectStatus']=='Reviewed' and $rFLists2['haltstudy']=='Yes' and $rFListsStudy['appealSubmitted']!='Yes'){?> 
   ...
   <?php }?>
     ...
     <?php if($rFLists2['projectStatus']=='Reviewed' and $rFLists2['haltstudy']=='Yes' and $rFListsStudy['appealSubmitted']=='Yes'){?><span class="button2"><?php echo $rFListsStudy['status'];?></span><?php }?>                                                   
