...
$studytoolsext_main1 = move_uploaded_file(preg_replace('/\s+/', '_', $_FILES['attachmentFile']['tmp_name']), $targetw1);
	...
	$sqlUsers="SELECT * FROM ".$prefix."grantcalls where grantID='$id' and category='concepts' order by grantID desc limit 0,1";
...
$mail->AddReplyTo("uncstuganda@gmail.com", "Call for Concept"); //To Address -- CHANGE --
/////////////////////////////Begin Mail Body
$mail->AddAddress("uncstuganda@gmail.com", "Call for Concept");
//$mail->addCc('$emailUsername','Activation Link from UNCST');//
$mail->addCc('mawandammoses@gmail.com','Call for Concept');//


$mail->WordWrap = 50; // set word wrap to 50 characters
$mail->IsHTML(false); // set email format to HTML
$mail->Subject = "Grants Management - Call for Concept";
$body="$allSentMessage
";
$mail->MsgHTML($body);
...
$mysqli->query($sqlA2);
echo '<meta http-equiv="Refresh" content="2; url='.$base_url.'main.php?option=CallConcepts" />';
}

if($id){
$sqlA2="update ".$prefix."grantcalls set  `summary`='$summary',`details`='$details',`startDate`='$startDate',`EndDate`='$EndDate',`category`='concepts' where `grantID`='$id'";
$mysqli->query($sqlA2);
echo '<meta http-equiv="Refresh" content="2; url='.$base_url.'main.php?option=CallConcepts" />';
}
...
?><div class="tab">
    ...
    <button class="tablinks"  onclick="openCity(event, 'conceptIntroduction')" id="defaultOpen"><?php echo $lang_CallforConcepts;?> </button>
