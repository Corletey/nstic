...
*/ 
$query = $mysqli->query("select COUNT(*) as num from ".$prefix."musers where (usrm_usrtype='superadmin'  || $session_usertype=='admin') order by usrm_fname asc");
...
/* Get data. */
$sql = "select *,DATE_FORMAT(`usrm_updated`,'%W, %D %M %Y') AS cfn_regdatem from ".$prefix."musers where (usrm_usrtype='superadmin'  || $session_usertype=='admin') order by usrm_fname asc LIMIT $start, $limitm";
