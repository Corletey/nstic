...
?>
<?php
if($_POST['doSaveData']=='Submit' and $_POST['actionConcept'] and $_POST['owner_id'] and $conceptID){
	...
	$wmproject="select * from ".$prefix."submissions_concepts where  owner_id='$owner_id' and conceptID='$conceptID'";
...
if($actionConcept=='Resubmission'){$projectStatus='Completeness Check-Resubmission';}
//'Pending Final Submission','Pending Review','',''
$sqlA2="update ".$prefix."submissions_concepts set `projectStatus`='$projectStatus',`rejectComents`='$rejectcomments' where owner_id='$owner_id' and conceptID='$conceptID'";
$mysqli->query($sqlA2);

if($actionConcept=='Resubmission'){
	
$sqlA23="update ".$prefix."submissions_concepts set `is_sent`='0',`finalSubmission`='Pending Final Submission'  where owner_id='$owner_id' and conceptID='$conceptID'";
$mysqli->query($sqlA23);	
}

$message='<p class="success">Dear '.$session_fullname.', concept has been '.$projectStatus.'.</p>';
logaction("$session_fullname $projectStatus Concept id:$conceptID");
...
$mail->WordWrap = 50; // set word wrap to 50 characters
$mail->IsHTML(false); // set email format to HTML
$mail->Subject = "Grants Management -  - Completeness Check (Rejected)";
$body="$allSentMessage
";
$mail->MsgHTML($body);
...
$sqlUsers2="SELECT * FROM ".$prefix."submissions_concepts where `conceptID`='$conceptID'";
...
<div class="tab">
   ...
   <button class="tablinks"onClick="window.location.href='./main.php?option=newreviewProjectInformation&conceptID=<?php echo $conceptID;?>&id=<?php echo $id;?>'"><?php echo $lang_new_ProjectInformation;?></button>
  <button class="tablinks" onClick="window.location.href='./main.php?option=newReviewconceptPrincipalInvestigator&conceptID=<?php echo $conceptID;?>&id=<?php echo $id;?>'"><?php echo $lang_new_ProjectTeam;?></button>
  <button class="tablinks" onClick="window.location.href='./main.php?option=newReviewconceptIntroduction&conceptID=<?php echo $conceptID;?>&id=<?php echo $id;?>'"><?php echo $lang_new_Introduction;?></button>
   <button class="tablinks" onClick="window.location.href='./main.php?option=newReviewconceptProjectDetails&conceptID=<?php echo $conceptID;?>&id=<?php echo $id;?>'"><?php echo $lang_new_ProjectDetails;?></button>
  <button class="tablinks"  onClick="window.location.href='./main.php?option=newReviewconceptBudget&conceptID=<?php echo $conceptID;?>&id=<?php echo $id;?>'"><?php echo $lang_new_Budget;?></button>
  <button class="tablinks"  onClick="window.location.href='./main.php?option=newReviewconceptReferences&conceptID=<?php echo $conceptID;?>&id=<?php echo $id;?>'"><?php echo $lang_new_Citations;?></button>
  <button class="tablinks"  onclick="openCity(event, 'newReviewconceptAction')" id="defaultOpen"><?php echo $lang_approve_rejectProcurement;?> </button>
...
<div class="container"><!--begin-->
  ...
  <div class="row success">
    ...
    <div class="col-100">
     ...
     <label for="fname"><input name="actionConcept" type="radio" value="RejectConcept" class="required" onChange="getApproveReject(this.value)"/><strong> <?php echo $lang_RejectConceptwithcomments;?></strong> <span class="error">*</span></label><br />
     
     <label for="fname"><input name="actionConcept" type="radio" value="Resubmission" class="required" onChange="getApproveReject(this.value)"/><strong> <?php echo $lang_Resubmission;?></strong> <span class="error">*</span></label>
